...
export function NewsCard({ post, variant = "default" }: NewsCardProps) {
      ...
      <article className="group">
        <Link href={`/news/${post.slug}`}>
          <div className="flex flex-col gap-3">
            ...
            <h2 className="text-xl md:text-2xl font-bold text-foreground group-hover:text-primary transition-colors leading-tight text-balance">
              {post.title}
            </h2>
  ...
  return (
    <article className="group border border-border rounded-lg p-5 bg-card hover:border-primary/50 transition-colors">
      ...
      <p className="text-sm text-muted-foreground line-clamp-2 mb-4">{post.content.substring(0, 120)}...</p>
